...
from utils.splatter_utils import (gt_attr_keys, 
                                  reconstruct_gaussians, 
                                  reconstruct_gaussians_batch,
                                  gs_render_batch)
...
class TrainingConfig:
    ...
    rendering_loss_weights: Dict[str, float]
    render_size: int
...
def log_validation(dataloader, vae, feature_extractor, image_encoder, unet, cfg: TrainingConfig, accelerator, weight_dtype, global_step, name, save_dir):
    ...
    for i, batch in enumerate(dataloader):
        ...
        task_embeddings = torch.cat([batch[f"{splatter_attr}_task_embeddings"] for splatter_attr in gt_attr_keys], dim=0)
            ...
            for guidance_scale in cfg.validation_guidance_scales:
                ...
                if rendering_loss_2dgs:
                    ...
                    if not batchify:
                        ...
                        gs_results = gs_render_batch(gs, gaussians, data, device=unet.device)
...
def main(
    ...
    vae = AutoencoderKL.from_pretrained(cfg.pretrained_model_name_or_path, subfolder="vae", revision=cfg.revision, torch_dtype=torch.float32)
    ...
    if cfg.pretrained_decoder_path is not None:
        ...
        # debug vae.encoder weights
        debug_vae = True
        if debug_vae:
            # compare vae.encoder weights before and after loading pretrained_decoder_path
            vae_encoder_weights_before = {name: param.clone() for name, param in vae.encoder.named_parameters()}
            
            # Prepare a set of parpameters that requires_grad=True in decoder
            trainable_decoder_params = set(f"encoder.{name}" for name, para in vae.encoder.named_parameters())
            # checked: this set is equal to check with model.vae.decoder.named_parameters(), whether dupplicate names
            ...
            # compare vae.encoder weights after loading pretrained_decoder_path
            vae_encoder_weights_after = {name: param.clone() for name, param in vae.encoder.named_parameters()}
            for name, param in vae_encoder_weights_before.items():
                if vae_encoder_weights_after[name] is None:
                    print(f"vae.encoder.{name} weights NOT EXIST after loading pretrained_decoder_path")
                    continue
    ...
    if cfg.trainable_modules is not None and 'vae.decoder' in cfg.trainable_modules:
        vae.decoder.requires_grad_(True)
        
        if len(cfg.trainable_modules) == 1:
            only_train_decoder = True
...
#    # check weight type of vae and unet
#     vae_weight_type = set()
#     for name, param in vae.named_parameters():
#         vae_weight_type.add(param.dtype)
#     unet_weight_type = set()
#     for name, param in unet.named_parameters():
#         unet_weight_type.add(param.dtype)
        
#     print("vae weight type: ", vae_weight_type) 
#     print("unet weight type: ", unet_weight_type) # torch.float32
    ...
    if only_train_decoder:
        ...
        st() # check decoder requires_grad
        if vae.decoder.requires_grad:
            optimizer.add_param_group({"params": vae.decoder.parameters(), "lr": cfg.learning_rate})
    ...
    # Move text_encode and vae to gpu and cast to weValueError: loaded state dict has a different number of parameter groupsight_dtype
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # repeat  (2B, Nv, 3, H, W)
                # imgs_out = torch.cat([normals_out, colors_out], dim=0)
                imgs_in = torch.cat([imgs_in]*num_domains, dim=0)
                imgs_out = torch.cat([batch[f"{splatter_attr}_out"] for splatter_attr in gt_attr_keys], dim=0) # (D B) V C H W
                ...
                camera_task_embeddings = torch.cat([camera_embeddings, task_embeddings], dim=-1)
                # print("camera_task_embeddings shape [main train loop]: ", camera_task_embeddings.shape)

                # (B*Nv, 3, H, W)
                imgs_in, imgs_out = rearrange(imgs_in, "B Nv C H W -> (B Nv) C H W"), rearrange(imgs_out, "B Nv C H W -> (B Nv) C H W")
                # print("imgs_in shape [main train loop]: ", imgs_in.shape, "imgs_out shape [main train loop]: ", imgs_out.shape)
                ...
                # debug = True
                # if debug:
                #     gaussians = batch['gaussians_recon']
                #     # gaussians = torch.load('/mnt/kostas_home/lilym/LGM/LGM/align_wonder3d_gaussians.pt').to(unet.device)
                #     # print("loading zero123++ gaussians")
                #     # st()

                # 3. render gaussians
                gs_results = gs_render_batch(gs, gaussians, batch, device=unet.device)
                for k, v in gs_results.items():
                    v = rearrange(v, "B V_render C H W -> (B V_render) C H W")
                    # print("gs_renderings: ", k, v.shape) # [20, 3, 256, 256]
                ...
                if only_train_decoder:
                    ...
                    loss += (rendering_loss_weights * rendering_loss).mean() # snr
                    print("keep the original latent loss")
                
                # 5. [Optional]: finetune decoder weights together with unet
                ...
                if accelerator.sync_gradients and cfg.max_grad_norm is not None:
                    ...
                    if 'vae.decoder' in cfg.trainable_modules:
                        accelerator.clip_grad_norm_(vae.decoder.parameters(), cfg.max_grad_norm)
                        # print("clip vae.decoder")   
                    if not only_train_decoder:
                        accelerator.clip_grad_norm_(unet.parameters(), cfg.max_grad_norm)
                        print("clip unet.decoder")   
                        st()
